...
class GraphPanel {
  ...
  html(webview) {
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${webview.cspSource}; script-src 'nonce-${nonce}' 'unsafe-eval';"><link rel="stylesheet" href="${asset("graph.css")}"></head><body><header><button id="chooseVault">Choose vault</button><button id="refresh">Refresh</button><button id="fit">Fit graph</button><button id="settingsButton" aria-expanded="false">Settings</button><span id="status">No vault selected</span></header><main><div id="graph" tabindex="0" aria-label="Obsidian vault graph"></div><p id="emptyState">No vault selected. Choose an Obsidian vault folder to render its links.</p></main><aside id="settings" hidden><div class="settings-header"><strong>Graph settings</strong><button id="closeSettings">← Back to graph</button></div><section><h2>Filters</h2><input id="search" type="search" placeholder="Search files"><label><input id="existing" type="checkbox"> Existing files only</label><label><input id="orphans" type="checkbox" checked> Show orphans</label></section><section><h2>Display</h2><label><input id="arrows" type="checkbox"> Arrows</label><label><input id="labels" type="checkbox" checked> Show labels</label><label>Text fade threshold<input id="textFadeThreshold" type="range" min="-3" max="3" step="0.1" value="0"><output id="textFadeThresholdValue" for="textFadeThreshold" class="range-value">0</output></label><label>Node size<input id="nodeSize" type="range" min="50" max="180" value="100"><output id="nodeSizeValue" for="nodeSize" class="range-value">100</output></label><label>Link thickness<input id="linkWidth" type="range" min="50" max="250" value="100"><output id="linkWidthValue" for="linkWidth" class="range-value">100</output></label><button id="animate" type="button">Animate</button></section><section><h2>Forces</h2><label>Center force<input id="centerForce" type="range" min="0" max="100" value="10"><output id="centerForceValue" for="centerForce" class="range-value">10</output></label><label>Repel force<input id="repelForce" type="range" min="0" max="180" value="100"><output id="repelForceValue" for="repelForce" class="range-value">100</output></label><label>Link force<input id="linkForce" type="range" min="0" max="100" value="100"><output id="linkForceValue" for="linkForce" class="range-value">100</output></label><label>Link distance<input id="linkDistance" type="range" min="40" max="320" value="100"><output id="linkDistanceValue" for="linkDistance" class="range-value">100</output></label></section><button id="reset">Restore default settings</button><p class="help">Scroll to zoom. Drag the background to pan. Drag a note to reposition it. Click a note to open it. Arrows show link direction. Animate replays notes appearing one by one.</p></aside><script nonce="${nonce}" src="${asset("pixi.min.js")}"></script><script nonce="${nonce}" src="${asset("d3.min.js")}"></script><script nonce="${nonce}" src="${asset("graph.js")}"></script></body></html>`;
  }
}
